...
from scalesim.memory.dram_mapping import dram_mapping
...
class read_buffer:
    def __init__(self):
        ...
        self.num_access_multibank = 0

        # Trace matrix
        self.trace_matrix = np.ones((1, 1))

        self.multibank_trace_Matrix=[]

        #DRAM Mapping
        self.DramMapping=dram_mapping()
    ...
    def set_params(self, backing_buf_obj,
                   ...
                   hit_latency=1, backing_buf_bw=1,num_banks=1
                   ):
    ...
    def set_fetch_matrix(self, fetch_matrix_np):
        ...
        num_lines_per_bank=num_lines//self.num_banks
    ...
    def prepare_hashed_buffer(self):
        ...
        self.num_lines_per_bank=num_lines_per_bank
    ...
    def prefetch_active_buffer(self, start_cycle):
        ...
        multibank=self.DramMapping(prefetch_requests)
        for i,bank in enumerate(multibank):
            multibank[i]=np.concatenate((response_cycles_arr, bank), axis=1)
        # Update the trace matrix
        self.multibank_trace_Matrix=multibank
    ...
    def new_prefetch(self):
        ...
        for i, bank in enumerate(multibank):
            multibank[i]= np.concatenate((response_cycles_arr, bank), axis=1)
            self.multibank_trace_Matrix[i]=np.concatenate((self.multibank_trace_Matrix[i],multibank[i]), axis=0)
    ...
    def get_multibank_trace_matrix(self):
        if not self.trace_valid:
            print('No trace has been generated yet')
            return

        return self.multibank_trace_Matrix
    #
    ...
    def print_multibank_trace(self, filename):
        if not self.trace_valid:
            print('No trace has been generated yet')
            return

        for i, bank in enumerate(self.multibank_trace_Matrix):
            np.savetxt(filename[:-4]+"_bank_"+str(i)+".csv", bank, fmt='%s', delimiter=",")